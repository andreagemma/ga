...
class Parallel:
    ...
    @staticmethod
    def get_num_cpus(num_cpus: int | float | None = None) -> int:
        if Parallel.initialzations > 0:
            return Parallel.num_cpus        
        if num_cpus is None:
            num_cpus_final = int(multiprocessing.cpu_count() - 1)
        elif isinstance(num_cpus, float):
            if 0 < num_cpus < 1:
                num_cpus_final = int(num_cpus * multiprocessing.cpu_count())
            elif -1 < num_cpus < 0:
                num_cpus_final = int((1+num_cpus) * multiprocessing.cpu_count())
            else:
                num_cpus = int(num_cpus)
        if isinstance(num_cpus, int):
            if num_cpus <= 0:  # If <0 then leaves the number of processors free
                num_cpus_final = int(multiprocessing.cpu_count() + num_cpus)
            else:  # If> 0 then use the number of processors indicated
                num_cpus_final = int(num_cpus)
        else:            
            num_cpus_final = int(multiprocessing.cpu_count() - 1)
        num_cpus_final = int(num_cpus_final)
        num_cpus_final = max(1, num_cpus_final) 
        num_cpus_final = min(num_cpus_final, multiprocessing.cpu_count())
        return num_cpus_final
    
    @staticmethod
    def get_num_min_cpus(num_cpus: int | float | None = None) -> int:        
        if Parallel.initialzations > 0:
    ...
    @staticmethod
    def initialize_parallel(num_cpus: Optional[int] = None, engine: Optional[str] = None, log: Optional[logging.Logger] = None, **kwargs: dict) -> int:
        if Parallel.log is None:
            if log is not None:
                Parallel.log = log
            else:
                log = logging.getLogger(Parallel.__class__.__name__)
